     ...
     <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="registration-form">
                ...
                <form method="POST" action="{{ route('updatereview') }}" class="form w-100" novalidate="novalidate" id="" >
                    @csrf
            ...
            @endif
                    ...
                    <h3 style="margin-bottom :25px;">Edit Review</h3>
                    </div>
                    <input type="hidden" class="form-control item" value="{{ $review->id }}" id="id" name="id" placeholder="Enter Name Of New User">
                    <div class="form-group">
                        <label for="username">Customer Name</label>
                        <input type="text" class="form-control item" value="{{ $review->name }}" id="name" name="name" placeholder="Enter Customer name">
                    </div>
                    <div class="form-group">
                        <label for="username">Customer Type</label>
                        <input type="text" class="form-control item" value="{{ $review->c_type }}" id="name" name="CustomerType" placeholder="Enter Customer Type">
                    </div>
                    <div class="form-group">
                        <label for="username">Title</label>
                        <input type="text" class="form-control item" value="{{ $review->title }}" id="name" name="title" placeholder="Enter Title of Review">
                    </div>
                    <div class="form-group">
                        <label for="username">Description</label>
                        <textarea type="text" class="form-control item"  id="description" rows="6" name="description" placeholder="Description">{{$review->description}}</textarea>
                    </div>
                    
                    
                    <select style="padding-top:6px;text-transform:capitalize;" name="rate" class="form-control item">
                            <option value="{{$review->rate}}">{{$review->rate}} Star</option>
